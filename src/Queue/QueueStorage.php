...
final class QueueStorage
{
    ...
    {
        try {
            $bytes = json_encode($message->payload, JSON_THROW_ON_ERROR);
    ...
    private function readRecordAt(mixed $fh, int &$offset): ?QueueMessage
    {
        ...
        $messageData = $this->readExactly($fh, $len);
        if (null === $messageData) {
            throw new CorruptedQueueException(
        ...
        try {
            $payload = json_decode($messageData, associative: true, flags: JSON_THROW_ON_ERROR);
        } catch (JsonException $e) {
            throw new CorruptedQueueException(
        ...
        return new QueueMessage(payload: $payload);
    }
}