...
final class IncomingMessageQueueRouterDmTest extends TestCase
{
    ...
    {
        $this->router->route($this->privateMessagePayload(12345678, '/settings'));

        $this->assertEnqueuedOnce('dm_12345678');
    }

    private function privateMessagePayload(int $userId, string $text): array
    {
        return [
            'update_id' => 100,
            'message' => [
    ...
    {
        $this->router->route($this->privateMessagePayload(12345, 'hello'));
